	...
	<view class="container">
		...
		<view class="book-details">
			...
			<view class="" style="position: relative;">
				<uni-icons @click="to_Illustrated" style="position: absolute;top: -130px;right:-20px" type="images-filled"
					size="50"></uni-icons>
				<text style="position: absolute;color: red;;font-size:15px; top:-90px;left:-20px;width: 40px;">图鉴</text>
			</view>
		</view>
		...
		<view class="book-description">
			...
			<p :class="{ 'expanded': isExpanded_book }" v-if="!isExpanded_book">{{ text_book }}...
				<a @click="isExpanded_book = !isExpanded_book" class="expand-link">{{ isExpanded_book ? '收起' : '展开' }}</a>
			</p>
			...
			<view class="ai-summary-section">
				<button @click="generateAIBookSummary" :disabled="generatingAiSummary" class="custom-button ai-button">
					<uni-icons v-if="!generatingAiSummary" type="sparkles" size="18" />
					{{ generatingAiSummary ? '生成中...' : 'AI生成个性化简介' }}
				</button>
			</view>

			<!-- AI生成的简介 -->
			<view v-if="showAiSummary" class="ai-summary">
				<h3>
		...
		<view class="author-biography">
			...
			<p :class="{ 'expanded': isExpanded_author }" v-if="!isExpanded_author">{{ text_author }}...
				<a @click="isExpanded_author = !isExpanded_author" class="expand-link">{{ isExpanded_author ? '收起' : '展开' }}</a>
			</p>
		...
		<view class="book-reviews">
			...
			<view class="review-item" v-for="(review, index) in book.Book_Comment" :key="index">
				<p><text style="color: blue;">{{ review.Comment_User_NickName }}</text> <text
						...
						@click="deleteCommentStart(review.Book_Comment_ID)" v-if="review.Comment_User_Name == user.username"
						style="float: right;" type="trash" size="15"></uni-icons></p>
...
// AI生成简介相关变量
let aiSummary = ref('');
let generatingAiSummary = ref(false);
let showAiSummary = ref(false);
...
// AI生成小说简介
async function generateAIBookSummary() {
	try {
		...
		const response = await uni.request({
			url: Get_AI_Book_Summary,
			method: 'POST',
			data: {
				prompt: book.value.Book_Introduce
			},
		...
		let errorMessage = '生成简介失败，请重试';

		// 根据错误类型提供更具体的提示
		if (error.errMsg && error.errMsg.includes('timeout')) {
			errorMessage = 'AI生成时间过长，请稍后再试';
